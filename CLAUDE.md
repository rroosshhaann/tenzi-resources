...
**Sheets:**
- `Events` — A=event, B=page, C=timestamp (Melbourne time), D=ip, E=referrer, F=site (`marketing` / `resources` / `email`), G=recipient. All page views, CTA clicks, dwell events, resources-site form submissions, and email-link click-tracking land here. Rows written before `track.js` shipped have an empty F. Rows older than the email-link redirect feature have an empty G. Site value `email` tags rows generated by the newsletter click-tracking redirect (`?redirect=`); the matching recipient identity sits in column G.
